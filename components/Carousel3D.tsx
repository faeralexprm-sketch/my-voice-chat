...
const Carousel3D: React.FC<Carousel3DProps> = ({ 
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      ...
      {/* Navigation Arrows */}
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-1 sm:px-12 z-50 pointer-events-none">
        <button 
            onClick={handlePrev}
            disabled={activeIndex === 0 || disabled}
            className={`p-2 sm:p-4 rounded-full bg-white/80 dark:bg-zinc-800/80 backdrop-blur-md shadow-lg border border-white/50 dark:border-zinc-700/50 text-zinc-800 dark:text-zinc-200 transition-all hover:scale-110 active:scale-95 pointer-events-auto focus-visible:ring-4 focus-visible:ring-indigo-500/50 outline-none ${activeIndex === 0 ? 'opacity-0 cursor-default' : 'opacity-100 hover:bg-white dark:hover:bg-zinc-700'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
            aria-label="Previous voice profile"
        >
        ...
        <button 
            ...
            className={`p-2 sm:p-4 rounded-full bg-white/80 dark:bg-zinc-800/80 backdrop-blur-md shadow-lg border border-white/50 dark:border-zinc-700/50 text-zinc-800 dark:text-zinc-200 transition-all hover:scale-110 active:scale-95 pointer-events-auto focus-visible:ring-4 focus-visible:ring-indigo-500/50 outline-none ${activeIndex === voices.length - 1 ? 'opacity-0 cursor-default' : 'opacity-100 hover:bg-white dark:hover:bg-zinc-700'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
            aria-label="Next voice profile"
        >
      ...
      <div 
        ref={containerRef} 
        className="relative w-full h-[60vh] sm:h-[70vh] flex items-center justify-center -mt-8 sm:mt-0"
        style={{ perspective: '1200px' }}
        role="region"
        aria-roledescription="carousel"
        aria-label="Voice selection carousel"
      >
        <div 
          ...
          <AnimatePresence initial={false}>
            {voices.map((voice, index) => {
              ...
              return (
                <motion.div
                  key={voice.name}
                  className={`group absolute w-[300px] sm:w-[360px] aspect-[3/4] bg-white dark:bg-zinc-800 rounded-3xl border border-zinc-100 dark:border-zinc-700 overflow-hidden cursor-pointer focus-visible:ring-4 focus-visible:ring-indigo-500/50 outline-none ${isActive ? 'shadow-2xl' : 'shadow-lg'}`}
                  style={{ zIndex }}
                  ...
                  role="listitem"
                  aria-roledescription="voice slide"
                  tabIndex={isActive && !disabled ? 0 : -1}
                  aria-label={`Voice card for ${voice.name}. ${voice.analysis.gender}, ${voice.pitch} pitch. ${isActive ? (isPlaying ? 'Currently playing. Press Enter or Space to pause.' : 'Selected. Press Enter or Space to play.') : 'Currently not selected. Click or press Arrow keys to navigate.'}`}
                  aria-current={isActive ? 'true' : 'false'}
                  aria-hidden={Math.abs(offset) > 1}
                >
                  {/* Focus Ring Indicator (Internal) */}
                  {isActive && (
                    <div className="absolute inset-0 rounded-3xl border-4 border-indigo-500/50 pointer-events-none z-[60] opacity-0 group-focus:opacity-100 transition-opacity"></div>
                  )}
