...
const VoiceFinder: React.FC<VoiceFinderProps> = ({ voices, onRecommendation, onClose }) => {
  ...
  const [sensitivity, setSensitivity] = useState<'exact' | 'broad'>('exact');
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        User Request: "${query}"
        Sensitivity Mode: ${sensitivity === 'exact' ? 'Prioritize technical specification matches.' : 'Prioritize stylistic and vibe similarities.'}
        ...
        <div className="relative p-8 sm:p-10">
          ...
          <div className="relative group">
            ...
            <div className="flex items-center gap-4 mt-6">
              <label className="text-sm font-semibold text-zinc-600 dark:text-zinc-400">Match Sensitivity:</label>
              <select 
                  value={sensitivity} 
                  onChange={(e) => setSensitivity(e.target.value as 'exact' | 'broad')}
                  className="bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500"
                >
                  <option value="exact">Exact Match</option>
                  <option value="broad">Broader Similarity</option>
              </select>
            </div>
